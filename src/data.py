...
def get_data_loaders(
    ...
    print(f"Total number of images in the training set: {len(train_data)}")
    ...
    if isinstance(sample_img, torch.Tensor):
        sample_img = sample_img.numpy().transpose((1, 2, 0))

    # Print the shape of the image
    print(f"Original image dimensions: {sample_img.shape}")

    # If you want to see the original size before any transforms
    original_img_path = train_data.samples[0][0]  # Path to the first image
    with Image.open(original_img_path) as img:
        print(f"Original image size before transforms: {img.size}")
    ...
    print(f"Number of training images used: {n_tot}")
